...
-- the states, gender affected and the confirmed cases in their respective states where confirmed cases are more than 100.
...
-- state that collected more than 1000 samples in a day
...
--the patient status in each state from the death_and_recovery table
select t1.State, t1.Patient_status,t2.City,t2.Age
from deathandrecovery t1
join deathandrecovery t2 on t1.State = t2.State
order by t1.State;

-- the hospital beds along with their location where patients have recovered from covid-19 and those beds are made available to the needy patients waiting in the queue to get admitted.
...
--The total number of people in assam who have recovered
select count(*) as TotalRecovered
from deathandrecovery
where State = 'Assam' and Patient_status = 'Recovered';

--the state, hospitals and beds available where population beds and hospitals available are more than 1000.
select State, Hospitals_Available,Beds_Available
from hospitalbeds
where Hospitals_Available > 1000 And Beds_Available >1000;

--States where active cases are less than 50
select State_UT
from statewisedata
where Active < 50;
...
--the details of the number of samples tested across each timestamp from the 'icmrtestingdata' table
select UpdatedTimeStamp,TotalSamplesTested
from icmrtestingdata;

--The number male and female who have recovered
...
--the total number of samples tested, total number of positive cases and the difference between 'TotalSamplesTested' and'TotalPostiveCases' columns from the icmrtestingdata' table
SELECT 
...
--the number of hospital beds available in each state,
SELECT State, SUM(Beds_Available) AS TotalBedsAvailable
FROM hospitalbeds
GROUP BY State;

--the total number of beds available in Tamil Nadu
SELECT SUM(Beds_Available) AS TotalBedsAvailable
FROM hospitalbeds
WHERE State = 'Tamil Nadu';

--Total number of beds available in India
SELECT SUM(Beds_Available) AS TotalBedsAvailable
FROM hospitalbeds;

-- the distinct values of 'TotalSamplesTested', 'TotalPositiveCases', and 'UpdatedTimeStamp' from the 'icmrtestingdata' table
SELECT DISTINCT TotalSamplesTested, TotalPositiveCases, UpdatedTimeStamp
FROM icmrtestingdata;

-- the total confirmed cases in Maharashtra until 31st March
SELECT SUM(Confirmed) AS TotalConfirmedCases
FROM statewisedata
WHERE State_UT = 'Maharashtra' AND Last_updad_time <= '2022-03-31';

-- the summing distribution of males and females aged 0 to 49 who have been impacted by COVID-19
SELECT 
...
--the recovery rate among the states and display it along with the names of the states and the number of recovered & active cases.
SELECT
    swd.State_UT,
...
--the states along with the ratio of Beds available against the total population beds
SELECT
    State,
    Beds_Available,
    Population_beds,
...
-- the different patient statuses and the corresponding cities recorded in the 'death_and_recovery' table, after joining it with the 'statewisedata' table based on the matching State_UT values
