...
--the average age of employees in each department and gender group.
SELECT department, gender, ROUND(AVG(age), 2) AS average_age
FROM employee
GROUP BY department, gender;

--the top 3 departments with the highest average training scores
SELECT
Top 3
...
--the percentage of employees who have won awards in each region
...
--the average length of service for employees in each department, considering only employees with previous year ratings greater than or equal to 4
SELECT department, ROUND(AVG(length_of_service), 2) AS average_length_of_service
FROM employee
WHERE previous_year_rating >= 4
GROUP BY department;

--the top 5 regions with the highest average previous year ratings
SELECT 
...
--the percentage of female employees who have won awards, per department. Also show the number of female employees who won awards and total female employees.
SELECT department,
       COUNT(CASE WHEN gender = 'Female' THEN 1 END) AS num_female_employees,
       COUNT(CASE WHEN gender = 'Female' AND awards_won = 1 THEN 1 END) AS num_female_employees_awarded,
       ROUND((COUNT(CASE WHEN gender = 'Female' AND awards_won = 1 THEN 1 END) * 100.0 / NULLIF(COUNT(CASE WHEN gender = 'Female' THEN 1 END), 0)), 2) AS percentage_female_awarded
FROM employee
GROUP BY department;

--the percentage of employees per department who have a length of service between 5 and 10 years.
SELECT department,
...
ORDER BY num_employees DESC;
 ...
 --the average length of service for employees per education level and gender, considering only those employees who have completed more than 2 trainings and have an average training score greater than 75 
...
--For each department and recruitment channel, the total number of employees who have met more than 80% of their KPIs, have a previous_year_rating of 5, and have a length of service greater than 10 years.
...
--the percentage of employees in each department who have received awards, have a previous_year_rating of 4 or 5, and an average training score above 70, grouped by department and gender
...
GROUP BY department, gender;
 ...
 --the top 5 recruitment channels with the highest average length of service for employees who have met more than 80% of their KPIs, have a previous_year_rating of 5, and an age between 25 and 45 years, grouped by department and recruitment channel.
